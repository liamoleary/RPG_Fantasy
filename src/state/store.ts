...
/**
 * The player's offer context (DN11 §3). One helper so the player rolls through
 * exactly the door the rivals do — a divergence here would be invisible and
 * would quietly make the camp unfair.
 */
const offerCtx = (p: Warlord): OfferContext => ({
...
export const useGame = create<Store>((set, get) => ({
  ...
  reroll: () => {
    ...
    p.camp = {
      ...p.camp,
      offer: rollOffer(offerCtx(p), makeUiRng(run, p.camp.rerollsUsedThisRound)),
  ...
  tierUp: () => {
    ...
    if (added > 0) {
      const rolled = rollOffer(offerCtx(p), makeUiRng(run, 900 + tier))
